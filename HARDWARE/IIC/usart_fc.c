...
{u8 i;	u8 sum = 0;
	...
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
	_temp = (vs16)( k_scale_pix*1000);//ultra_distance;
	data_to_send[_cnt++]=BYTE1(_temp);
	data_to_send[_cnt++]=BYTE0(_temp);
...
u8 mav_flow_sel;
FLOW flow;
FLOW_RAD flow_rad,flow_rad_mine;
...
//			 sonar_new=1;
//		 else
		// sonar_new=ByteToFloat(floattobyte);//ground_distance	float	Ground distance in m. Positive value: distance known. Negative value: Unknown distance     
