...
u8 qual;
	...
	get_time_between_images = Get_Cycle_T(0);
	//get_time_between_images=0.075;
...
pixel_flow_y_gro_fix=LIMIT((my_deathzoom(flow_rad.integrated_xgyro,dead_gro)*10000*scale_grox),LIMIT(-fabs(pixel_flow_x_pix),-1000,-0.5),LIMIT(fabs(pixel_flow_x_pix),0.5,1000));
...
pixel_flow_y=pixel_flow_y_pix;//Moving_Median(1,3,);//s2.x;
  pixel_flow_x_gro_fix=LIMIT((my_deathzoom(flow_rad.integrated_ygyro,dead_gro)*10000*scale_grox),LIMIT(-fabs(pixel_flow_y_pix),-1000,-0.5),LIMIT(fabs(pixel_flow_y_pix),0.5,1000));
...
if(ultra_distance==0)
	...
	ultra_distance=4000;
	f_com[0]=flow_compx*((float)(ultra_distance)/1000.)*scale_acc/10;
	f_com[1]=flow_compy*((float)(ultra_distance)/1000.)*scale_acc/10;
