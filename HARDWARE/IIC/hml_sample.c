...
//u8 ak8975_buffer[6]; //接收数据缓存
  ...
  ak8975.Mag_Adc.y= IIR_I_Filter(mag_temp[1], InPut_IIR_hml[1], OutPut_IIR_hml[1], b_IIR_hml, IIR_ORDER+1, a_IIR_hml, IIR_ORDER+1);
	...
	ak8975.Mag_Adc.z= IIR_I_Filter(mag_temp[2]-500, InPut_IIR_hml[2], OutPut_IIR_hml[2], b_IIR_hml, IIR_ORDER+1, a_IIR_hml, IIR_ORDER+1);
	ak8975.Mag_Val.x = (ak8975.Mag_Adc.x - ak8975.Mag_Offset.x)*ak8975.Mag_Gain.x ;
		...
		if(ABS(ak8975.Mag_Adc.x)<700&&ABS(ak8975.Mag_Adc.y)<700&&ABS(ak8975.Mag_Adc.z)<700)
		{
			...
			{ init=0;
			  ...
			  if( MagSum.z>temp_max)
				...
				ak8975.Mag_Gain.x =  temp_max/MagSum.x ;
				ak8975.Mag_Gain.y =  temp_max/MagSum.y ;
				ak8975.Mag_Gain.z =  temp_max/MagSum.z ;
