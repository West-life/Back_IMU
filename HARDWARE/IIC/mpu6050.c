...
//	IIC_Write_1Byte(MPU6050_ADDR,MPU6050_RA_PWR_MGMT_1, 0x80);
	...
	Delay_ms(10);
	#if ACC_IS_16G
	MPU6050_setFullScaleAccelRange(MPU6050_ACCEL_FS_16);	//加速度度最大量程 +-8G
	#else
...
//10 170 4056
	...
	#if ACC_IS_16G
	acc_scale[i]=(float)(4096/2*2)/(acc_max[i]-acc_min[i]);
  #else		
...
//	Transform(mpu_fil_tmp_acc_circle[A_X],mpu_fil_tmp_acc_circle[A_Y],mpu_fil_tmp_acc_circle[A_Z],&mpu6050.Acc_c.x,&mpu6050.Acc_c.y,&mpu6050.Acc_c.z);
  ...
  imu_fushion.Acc.x=mpu6050.Acc.x;
	...
	imu_fushion.Acc.z=mpu6050.Acc.z;
	imu_fushion.Gyro.x=mpu6050.Gyro.x;
	imu_fushion.Gyro.y=mpu6050.Gyro.y;
	imu_fushion.Gyro.z=mpu6050.Gyro.z;
	imu_fushion.Gyro_deg.x=mpu6050.Gyro_deg.x;
	imu_fushion.Gyro_deg.y=mpu6050.Gyro_deg.y;
	imu_fushion.Gyro_deg.z=mpu6050.Gyro_deg.z;
	imu_fushion.Mag_Val.x=ak8975.Mag_Val.x;
	imu_fushion.Mag_Val.y=ak8975.Mag_Val.y;
	imu_fushion.Mag_Val.z=ak8975.Mag_Val.z;
	imu_fushion.Alt=(float)baroAlt/1000.;
#endif
